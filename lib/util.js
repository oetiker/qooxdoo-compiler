 ...
 */
(function() {
  var LM = require("./qxcompiler/utils/LogManager").getInstance();
...
module.exports.json = {
  parseISO : function(str) {
    ...
    var year = parseInt(m[1], 10), month = parseInt(m[2], 10) - 1, day = parseInt(m[3], 10), hours = parseInt(m[4], 10), minutes = parseInt(
        m[5], 10), seconds = parseInt(m[6], 10), millis = parseInt(m[7], 10);
  ...
  async.eachSeries(
      ...
      function(seg, cb) {
        ...
        fs.exists(made, function(exists) {
          if (!exists) {
            return fs.mkdir(made, function(err) {
          ...
          fs.stat(made, function(err, stat) {
            ...
            return cb(new Error("Cannot create " + made + " (in " + dir + ") because it exists and is not a directory", "ENOENT"));
          });
        });
      },
      function(err) {
