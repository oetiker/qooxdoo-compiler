            ...
            async.parallel([
                function(callback) {
                  ...
                  fs.readFile(analyser.getQooxdooPath() + "/../tool/data/generator/loader.tmpl.js", { encoding: "utf-8" }, function(err, data) {
                    ...
                    data = lines.join('\n');
                    fs.writeFile(analyser.getBuildType() + path.sep + "script" + path.sep + "boot.js", data, callback);
                  });
                },
                
                function(callback) {
                  fs.writeFile(analyser.getBuildType() + path.sep + "index.html",
                    "<!DOCTYPE html>\n" + 
                    "<html>\n" +
                    "<head>\n" +
                ...
                function(callback) {
                  fs.writeFile(analyser.getBuildType() + path.sep + "script" + path.sep + "test.json", JSON.stringify(configdata, null, 2), callback);
                },
                
                function(callback) {
                  fs.writeFile(analyser.getBuildType() + path.sep + "script" + path.sep + "resources.js", "qx.$$packageData['0'] = " + JSON.stringify(pkgdata, null, 2) + ";\n", callback);
                },
                
                function(callback) {
                  fs.writeFile(analyser.getBuildType() + path.sep + "script" + path.sep + "resources.json", JSON.stringify(pkgdata, null, 2) + "\n", callback);
                }
              ],
              callback);
          });
