  ...
  members: {
    ...
    open: function(cb) {
      ...
      if (this.isProcessResources()) {
        ...
        var resDb;
        if (m && m.length == 3)
          resDb = m[1] + path.sep + "resource-db.json";
        else
    ...
    getClassFilename: function(className) {
      ...
      if (!library)
        return null;
      var path = library.getRootDir() + path.sep + library.getSourcePath() + path.sep + className.replace(/\./g, path.sep) + ".js";
      return path;
    },
    ...
    getQooxdooVersion: function() {
      ...
      var data = fs.readFileSync(path.normalize(this.getQooxdooPath() + "/../version.txt"), { encoding: "utf-8" });
      if (data)
