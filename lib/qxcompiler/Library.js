...
qx.Class.define("qxcompiler.Library", {
  ...
  members: {
    ...
    loadManifest: function(rootDir, cb) {
      ...
      fs.readFile(path.normalize(rootDir + "/Manifest.json"), { encoding: "utf8" }, function(err, data) {
        if (!err) {
    ...
    scanForClasses: function(cb) {
        ...
        fs.readdir(folder, function(err, filenames) {
          ...
          async.each(
              ...
              function(filename, cb) {
                if (filename[0] == '.')
                  return cb();
                fs.stat(path.normalize(folder + "/" + filename), function(err, stat) {
                  if (!stat)
                    return cb();
                  ...
                  if (stat.isDirectory()) {
                    ...
                    if (tmp.length)
                      tmp += ".";
                    tmp += filename;
                    return scanDir(path.normalize(folder + "/" + filename), tmp, cb);
                  }
      ...
      scanDir(path.normalize(t.getRootDir() + "/" + t.getSourcePath()), "", function(err) {
        cb(err, classes);
      });
    },
    ...
    __getSymbolTypeImpl: function(name, cb) {
      ...
      function test(testPath, cb) {
        testPath = path.normalize(t.getRootDir() + "/" + t.getSourcePath() + "/" + testPath);
      ...
      async.until(
          ...
          function(cb) {
            var testPath = getSymbol(index, '/');
            test(testPath, function(err, result) {
              if (result === "file")
                found(testPath, "resource", cb);
              else if (result === "testPath")
                found(testPath, "package", cb);
              else
                test(testPath + ".js", function(err, result) {
                  if (result == "file")
                    found(testPath, "class", cb);
                  else
                    cb();
                });
            });
