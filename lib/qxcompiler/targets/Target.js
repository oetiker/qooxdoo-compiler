...
module.exports = qx.Class.define("qxcompiler.targets.Target", {
  ...
  members: {
    ...
    generateApplication: function(application, environment, cb) {
      ...
      var namespace = compileInfo.namespace = qxcompiler.ClassFile.getNamespace(application.getClassName());
      var library = compileInfo.library = analyser.findLibrary(namespace);
    ...
    _writeApplication: function(compileInfo, cb) {
      ...
      async.parallel([
          function (cb) {
            ...
            fs.readFile(path.normalize(analyser.getQooxdooPath() + "/../tool/data/generator/loader.tmpl.js"),
                {encoding: "utf-8"},
                function (err, data) {
                  if (err)
                    return cb(err);
                  ...
                  data = lines.join('\n');
                  var ws = fs.createWriteStream(path.normalize(appRootDir + application.getName() + "/" + t.getScriptPrefix() + "boot.js"));
                  ws.write(data);
          ...
          function (cb) {
            fs.writeFile(path.normalize(appRootDir + application.getName() + "/" + t.getScriptPrefix() + "resources.js"),
          ...
          function (cb) {
            fs.writeFile(path.normalize(appRootDir + application.getName() + "/" + t.getScriptPrefix() + "resources.json"),
    ...
    _writeIndexHtml: function(compileInfo, cb) {
      ...
      if (!t.isGenerateIndexHtml())
        return cb();
      fs.writeFile(path.normalize(appRootDir + t.getScriptPrefix() + application.getName() + ".html"),
          "<!DOCTYPE html>\n" +
          "<html>\n" +
          "<head>\n" +
