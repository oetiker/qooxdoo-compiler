            ...
            async.eachSeries(uris,
                function (uri, cb) {
                  ...
                  if (!currentNamespace || currentNamespace != namespace) {
                    ...
                    ws = fs.createWriteStream(path.normalize(t.getOutputDir() + "/" + application.getName() + "/" + outputFilename));
                    package.uris.push("__out__:" + outputFilename);
                    currentNamespace = namespace;
                  }
                  var rs = fs.createReadStream(path.normalize(library.getRootDir() + "/" + library.getSourcePath() + "/" + filename));
                  rs.on('end', function () {
                    ws.write("\n");
                    cb();
                  });
