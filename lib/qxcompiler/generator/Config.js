...
qx.Class.define("qxcompiler.generator.Config", {
  ...
  members: {
    ...
    load: function (callback) {
      this.rootConfig = new qxcompiler.generator.ConfigFile(this, this.__filename);
    ...
    getLibrary: function (path) {
      ...
      if (json.provides.namespace == "qx") {
        path = path.normalize(rootDir + "/../version.txt");
        ...
        if (job.run) {
          job.run.forEach(function (jobName) {
            ...
            if (!childJob)
              ...
              childJob = qxcompiler.generator.ConfigFile.clone(childJob);
              var parentJobClone = qxcompiler.generator.ConfigFile.clone(job);
