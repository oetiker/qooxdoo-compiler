...
qx.Class.define("qxcompiler.ClassFile", {
        ...
        CallExpression(path) {
          ...
          if (types.isMemberExpression(path.node.callee)) {
            ...
            if (name == "qx.core.Environment.add") {
              ...
              if (!arg0)
                log.warn("Cannot interpret argument[0] in " + name + "() at " + lineNo);
              else
              ...
              if (!arg0 || !arg1)
                log.warn("Cannot interpret arguments in " + name + "() at " + lineNo);
              else
  ...
  statics: {
    ...
    getSourcePath: function(library, className) {
      return library.getRootDir() + path.sep + library.getSourcePath() + path.sep + className.replace(/\./g, path.sep) + ".js";
    },
    ...
    getOutputPath: function(analyser, className) {
      return analyser.getOutputDir() + "transpiled" + path.sep + className.replace(/\./g, path.sep) + ".js";
    },
