 ...
 */
module.exports = qx.Class.define("qxcompiler.Translation", {
  ...
  members: {
    ...
    getPoFilename: function() {
      ...
      return path.normalize(library.getRootDir() + "/" + library.getTranslationPath() + "/" + this.getLocale() + ".po");
    },
      ...
      fs.exists(poFile, function(exists) {
        ...
        fs.stat(poFile, function(err, stat) {
          ...
          fs.readFile(poFile, { encoding: "utf8" }, function(err, data) {
            ...
            var onRead = t.__onRead;
            t.__onRead = null;
            t.__reading = false;
            onRead.forEach(function(cb) { cb(); });
          });
        });
      });
    },
