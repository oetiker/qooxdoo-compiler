  ...
  members: {
    ...
    _initialise: function(cb) {
      ...
      function addLibrary(namespace, rootDir, cb) {
        ...
        lib.loadManifest(path.normalize(rootDir), function(err) {
          if (!err)
    ...
    make: function(cb) {
      ...
      async.series([
           ...
           */
          function(cb) {
            fs.readFile(path.normalize(t.__qooxdooPath + "/application/demobrowser/source/script/demodata.json"), {encoding: "utf8"}, function(err, data) {
              if (err)
                return cb(err);
              t.__demodata = JSON.parse(data);
              cb();
            });
          },
          ...
          function(cb) {
            ...
            DEMO_APP_CLASSES.forEach(function(name) {
              appInfos.push({
                app: new qxcompiler.Application(name, [
                  "qx.theme.Indigo",
                  "qx.theme.Simple",
                  "qx.theme.Modern",
                  "qx.theme.Classic",
            ...
            appInfos.push({
              app: new qxcompiler.Application("demobrowser.Application", [
                  "demobrowser.Theme",
                  "qx.theme.Indigo",
                  "qx.theme.Simple",
                  "qx.theme.Modern",
            ...
            async.parallel([
              ...
              function(cb) {
                qxcompiler.files.Utils.sync(t.__qooxdooPath + "/application/demobrowser/source/resource",
                    target.getOutputDir() + "/resource", cb);
              },
              ...
              function(cb) {
                async.each(DEMO_APP_CLASSES, function(name, cb) {
                  ...
                  qxcompiler.files.Utils.sync(filename, target.getOutputDir() + "/script/" + name + ".src.js", cb);
                }, cb);
              },
          ...
          function(cb) {
            ...
            fs.readFile("demobrowser-helper.js", { encoding: "utf-8" }, function(err, data) {
              if (err)
                return cb(err);
              fs.writeFile(path.normalize(target.getOutputDir() + "/demo/helper.js"), data, { encoding: "utf8" }, function(err) {
                return cb(err);
              });
            });
          }
